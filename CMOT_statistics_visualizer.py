...
def all_location_distribution(save_path):
    ...
    ax.imshow(heatmap, origin='upper', cmap=cm.jet) # 'upper' indicates (0,0) at the left upper corner
...
def location_distribution_per_category(save_path):
    ...
    for category in all_category:
        ...
        ax.get_yaxis().set_ticks([])
        ax.axis('off')
        plt.savefig(save_path + '6-location-distribution_'+category+'.jpg', bbox_inches='tight')
